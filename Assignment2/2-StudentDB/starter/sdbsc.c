...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t existing;
    if (read(fd, &existing, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
...
int del_student(int fd, int id)
{
    ...
    int result = get_student(fd, id, &existing);
    ...
    // delete by overwriting with zeroes.
...
int print_db(int fd)
{
    ...
    student_t student;
    int header_printed = 0;     // 0 meaning false and 1 meaning true
    int result;

    while(1) {
        result = read(fd, &student, STUDENT_RECORD_SIZE);
        ...
        if (memcmp (&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            ...
            float calculate_gpa = student.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, calculate_gpa);
        }
    }

    if (header_printed == 0) {      // if header is not printed, it means database is empty.
        printf(M_DB_EMPTY);
    }
...
void print_student(student_t *s)
{
    if (s == NULL && s -> id <= 0) {
        ...
        float calculate_gpa = s -> gpa / 100.0;
        printf(STUDENT_PRINT_FMT_STRING, s -> id, s -> fname, s -> lname, calculate_gpa);
    }
}
