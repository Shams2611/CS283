...
int exec_remote_cmd_loop(char *address, int port){
    ...
    char *cmd_buffer = malloc(SH_CMD_MAX);
    char *response_buffer = malloc(RDSH_COMM_BUFF_SZ);
    if(!cmd_buffer || !response_buffer){
        client_cleanup(client_sock, cmd_buffer, response_buffer, ERR_MEMORY);
        return ERR_MEMORY;
    }
    while (1){
        printf("%s", SH_PROMPT);
    ...
    client_cleanup(client_sock, cmd_buffer, response_buffer, OK);
    return OK;
}
